...
// MADEIRA: guest window helpers. See JITClass.h for the contract these implement.
...
DEF_OP(LoadMemPair) {
  ...
  // MADEIRA: Op->Offset here is a u32 ldp immediate, not an IR operand, so GetGuestMemAddr cannot
...
DEF_OP(LoadMemTSO) {
  ...
  // MADEIRA: with a guest window this folds the displacement into the address and hands back an
...
DEF_OP(VLoadVectorMasked) {
  ...
  // MADEIRA: the non-SVE fallback below rebuilds working pointers out of MemReg with TMP2, so MemReg
...
DEF_OP(VStoreVectorMasked) {
  ...
  const auto MaskReg = GetVReg(Op->Mask);
  // MADEIRA: as in VLoadVectorMasked, the non-SVE fallback derives working pointers from MemReg.
  const auto Guest = GetGuestMemAddr(OpSize, Op->Addr, Op->Offset, Op->OffsetType, Op->OffsetScale);
  const auto MemReg = Guest.Base;
  if (HostSupportsSVE128 || HostSupportsSVE256) {
...
void Arm64JITCore::Emulate128BitGather(IR::OpSize Size, IR::OpSize ElementSize, ARMEmitter::VRegister Dst,
  ...
  for (size_t i = DataElementOffsetStart, IndexElement = IndexElementOffsetStart; i < NumDataElements; ++i, ++IndexElement) {
    ...
    // MADEIRA: the gather address has now been formed *completely* in the guest namespace, at
...
DEF_OP(VLoadVectorGatherMasked) {
  ...
  // MADEIRA: !GuestBase is belt-and-braces. The SVE form builds `[Xbase, Zindex]` in hardware, so the
...
DEF_OP(VLoadVectorGatherMaskedQPS) {
  ...
  ///< If the host supports SVE and the offset scale matches SVE limitations then it can do an SVE style load.
  // MADEIRA: see the note on VLoadVectorGatherMasked for why !GuestBase is here.
...
DEF_OP(Push) {
  ...
  if (GuestBase) {
    // MADEIRA: the identity-mapped path below uses pre-indexed addressing, whose writeback would
...
DEF_OP(PushTwo) {
  ...
  if (GuestBase) {
    // MADEIRA: same split as Push - decrement the architectural pointer in the guest namespace,
...
DEF_OP(StoreMemPair) {
  ...
  const auto OpSize = IROp->Size;
  // MADEIRA: see the note in LoadMemPair about Op->Offset being a bounded non-negative stp
...
DEF_OP(MemSet) {
  ...
  if (GuestBase) {
    // MADEIRA: TMP2 is the working pointer the whole memset walks, and it is the only thing here
...
DEF_OP(MemCpy) {
  ...
  if (GuestBase) {
    // MADEIRA: TMP2/TMP3 are the two working pointers that actually get dereferenced, so both are
...
DEF_OP(VStoreNonTemporal) {
  ...
  const auto Value = GetVReg(Op->Value);
  // MADEIRA: Op->Offset is an i8 structure immediate, not an IR operand; see LoadMemPair.
  const auto MemReg = GetGuestMemReg(Op->Addr);
